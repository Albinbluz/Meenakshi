...
const {Message, StringSession, Image, Video} = require('./Meenakshi/');
...
var PROP2 = { vishnuser: '916235989299,0' }
var string = base + `vishnuserkl29`
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        console.log(
            chalk.green.bold('✅ WA Connection successful!')
        );
        ...
        console.log(
         ...
         throw new Error("Incorrect Password Please Contact Developer ⚠ ");         
         return; //created by vishnuserkl29
         }

        console.log(
            chalk.blueBright.italic('⬇️ Downloading external plugins from /root/Aishwarya...')
        );
        ...
        console.log(
            chalk.green.bold('✅ WhatsMeenu is connected!')
        );
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ *ERROR REPORT : WHATSMEENU!* ~______~*' +
                                    '\n\n*🙊 ' + error + '*\n\n```*ASK Developer to report this issue wa.me/916235989299* ' 
                                    , MessageType.text);
                            }
                        }
                    }
